...
input = [int(line) for line in open(f'{os.path.dirname(__file__)}/input.txt', 'r').readlines()]

pair = find_pair(values=input, target=2020)
print(f"{pair[0]} * {pair[1]} = {pair[0] * pair[1]}")

triple = find_triple(values=input, target=2020)
